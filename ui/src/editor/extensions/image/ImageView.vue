...
<template>
  <InlineBlockBox>
    <div
      ref="resizeRef"
      class=":uno: group relative inline-block max-w-full overflow-hidden rounded-md text-center"
      :class="{
        ':uno: rounded ring-2': selected,
        ':uno: resize-x': !initialization,
      }"
      :style="{
    ...
    >
      <div v-if="src || fileBase64" class=":uno: relative">
        <img
          :src="src || fileBase64"
        ...
        <div
          v-if="fileBase64"
          class=":uno: absolute top-0 h-full w-full bg-black bg-opacity-20"
        >
